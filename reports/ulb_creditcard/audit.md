...
| T1.2 Amount-only share of no-ID PR-AUC | PASS | 0.01447 | >= 0.8 | Compares amount-only average precision to the no-ID baseline. This dependency-free ratio is screening-only; confirm it with the optional LightGBM sensitivity model. |
| T1.3 Best one-sided amount threshold | PASS | {'f1': 0.032490974729241874, 'f1_ratio': 0.07230834572095986} | F1 >= 0.7 and ratio >= 0.8 | Training-selected threshold rule: Amount: amount <= 0; evaluated unchanged on the provided validation split. The ratio compares against the no-ID baseline's best score-threshold F1. This dependency-free ratio is screening-only; confirm it with the optional LightGBM sensitivity model. |
| T2 Single-feature shortcut | WARN | {'column': 'Amount', 'value': '1', 'count': 13688, 'positive_rate': 0.00825540619520748, 'recall': 0.22967479674796748, 'lift': 4.778856650891172} | recall >= 0.2 and lift >= 3.0 | Broad shortcut warning: Amount=1 has positive_rate 0.8%, lift 4.8x, recall 23.0% (n=13,688). |
...
| T4.2 Entity-holdout degradation | PASS | {'ap_lift_ratio': 1.5702897669646978, 'raw_ap_ratio': 2.015148751436853, 'provided_prevalence': 0.0012640005617780275, 'holdout_prevalence': 0.0016220886154063156} | > 0.75 | Compares prevalence-normalized AP-lift on entity holdout against the provided split. |
...
| T7 Temporal split degradation | PASS | {'ap_lift_ratio': 1.3712617589087543, 'raw_ap_ratio': 1.6347915270658464, 'provided_prevalence': 0.0012640005617780275, 'temporal_prevalence': 0.0015069168196199141} | > 0.75 | Compares prevalence-normalized AP-lift on temporal holdout against the provided split. |
...
- provided_split_no_id_nb: ROC-AUC 0.9656, PR-AUC 0.272, Best F1 0.4493, Top 1% P=10.07%, R=79.63%
- provided_split_id_only_nb: ROC-AUC 0.5, PR-AUC 0.001305, Best F1 0.002525, Top 1% P=0.12%, R=0.93%
- provided_split_with_id_nb: ROC-AUC 0.9656, PR-AUC 0.272, Best F1 0.4493, Top 1% P=10.07%, R=79.63%
- provided_split_amount_only_nb: ROC-AUC 0.7196, PR-AUC 0.003938, Best F1 0.01986, Top 1% P=1.17%, R=9.26%
- provided_split_amount_threshold: `Amount` amount <= 0, train F1 0.02042, validation F1 0.03249
- temporal_holdout_no_id_nb: ROC-AUC 0.9715, PR-AUC 0.4447, Best F1 0.5116, Top 1% P=12.36%, R=81.99%
- temporal_holdout_amount_only_nb: ROC-AUC 0.6728, PR-AUC 0.00413, Best F1 0.01846, Top 1% P=0.86%, R=5.69%
- entity_holdout_no_id_nb: ROC-AUC 0.9696, PR-AUC 0.5482, Best F1 0.5926, Top 1% P=12.87%, R=79.35%
- entity_holdout_id_only_nb: ROC-AUC 0.5, PR-AUC 0.002265, Best F1 0.003239, Top 1% P=0.00%, R=0.00%
...
- [WARN][T2] Broad shortcut warning: Amount=1 has positive_rate 0.8%, lift 4.8x, recall 23.0% (n=13,688).
